...
HTML4_NAMES_TO_HEX = {u'aqua': u'#00ffff', u'black': u'#000000', u'blue': u'#0000ff', u'fuchsia': u'#ff00ff', u'green': u'#008000', u'gray': u'#808080', u'lime': u'#00ff00', u'maroon': u'#800000', u'navy': u'#000080', u'olive': u'#808000', u'purple': u'#800080', u'red': u'#ff0000', u'silver': u'#c0c0c0', u'teal': u'#008080', u'white': u'#ffffff', u'yellow': u'#ffff00',}
...
#################################################################
(html4_names_to_hex, css2_names_to_hex, css21_names_to_hex, css3_names_to_hex) = (HTML4_NAMES_TO_HEX, CSS2_NAMES_TO_HEX, CSS21_NAMES_TO_HEX, CSS3_NAMES_TO_HEX)

(html4_hex_to_names, css2_hex_to_names, css21_hex_to_names, css3_hex_to_names) = (HTML4_HEX_TO_NAMES, CSS2_HEX_TO_NAMES, CSS21_HEX_TO_NAMES, CSS3_HEX_TO_NAMES)
...
def normalize_hex(hex_value):
    ...
    if match is None:
        raise ValueError(u"'%s' is not a valid hexadecimal color value." % hex_value)
...
def name_to_hex(name, spec=u'css3'):
    ...
    hex_value = {u'css2': CSS2_NAMES_TO_HEX, u'css21': CSS21_NAMES_TO_HEX, u'css3': CSS3_NAMES_TO_HEX, u'html4': HTML4_NAMES_TO_HEX}[spec].get(normalized)
    if hex_value is None:
        raise ValueError(u"'%s' is not defined as a named color in %s." % (name, spec))
    return hex_value
...
def hex_to_name(hex_value, spec=u'css3'):
    ...
    if name is None:
        raise ValueError(u"'%s' has no defined color name in %s." % (hex_value, spec))
    return name
...
def hex_to_rgb(hex_value):
    ...
    return (hex_value >> 16, hex_value >> 8 & 0xff, hex_value & 0xff)
...
def rgb_to_rgb_percent(rgb_triplet):
    ...
    # special-case them.
    specials = {255: u'100%', 128: u'50%', 64: u'25%', 32: u'12.5%', 16: u'6.25%', 0: u'0%'}
    return tuple(specials.get(d, u'%.02f%%' % (d / 255.0 * 100)) for d in normalize_integer_triplet(rgb_triplet))
...
def html5_parse_simple_color(input):
    ...
    if not isinstance(input, unicode) or len(input) != 7:
        raise ValueError(u"An HTML5 simple color must be a Unicode string "
    ...
    if not input.startswith('#'):
        raise ValueError(u"An HTML5 simple color must begin with the "
                         u"character '#' (U+0023).")
    ...
    if not all(c in string.hexdigits for c in input[1:]):
        raise ValueError(u"An HTML5 simple color must contain exactly six ASCII hex digits.")
    ...
    #
    # 9. Return result.
    result = (int(input[1:3], 16), int(input[3:5], 16), int(input[5:7], 16))
    return result
...
def html5_parse_legacy_color(input):
    ...
    if not isinstance(input, unicode):
        raise ValueError(u"HTML5 legacy color parsing requires a Unicode string as input.")
    ...
    if input.lower() == u"transparent":
        raise ValueError(u'HTML5 legacy color parsing forbids "transparent" as a value.')
    ...
    #    substeps:
    if len(input) == 4 and input.startswith(u'#') and all(c in string.hexdigits for c in input[1:]):
        ...
        #    number multiplied by 17.
        result = (int(input[1], 16) * 17, int(input[2], 16) * 17, int(input[3], 16) * 17)
    ...
    input = ''.join(u'00' if c > 0xffff
                    else unichr(c) for c in codepoints)
    ...
    result = (int(red, 16), int(green, 16), int(blue, 16))
